...
import { QRCodePanel2 } from "./qr-code-panel-2"
...
export const ApprovalLetterManager2 = ({ slug, documentData }: Params) => {
  ...
  const approveDocumentHandle = async () => {
    ...
    try {
      ...
      const res = await fetch(`${API_V1_BASE_URL}/documents/${slug}/confirm`, {
        method: 'POST',
        cache: 'no-store', // Or 'force-cache' if data is not updated often
        headers: {
          'Authorization': `Bearer ${session.data?.accessToken}`,
      ...
      setIsLoading(false)
      router.push('/jagratama/documents')
    } catch (error) {
      console.error("Error approving document:", error)
      toast.error(`Document create failed`)
      setIsLoading(false)
    }
  }
